...
export class Orders implements OnInit {
 ...
 if (existing) {
 return items.map(i => i.product.code === code ? { ...i, quantity: i.quantity + qty } : i);
 }
 ...
 this.showCreate.set(false);
 this.successMessage.set(`Pedido #${res.orderNumber} creado. Total: $ ${res.total.toFixed(2)}.`);
 this.loadOrders();
 },
 error: (err) => {
 this.creating.set(false);
 this.createError.set(err.error?.message ?? 'No se pudo crear el pedido. Revisá los datos.');
 }
 });
 }
