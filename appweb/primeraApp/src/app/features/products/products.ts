...
function jpgImagesValidator(control: AbstractControl): ValidationErrors | null {
 ...
 const entries = value.split(',').map((e: string) => e.trim()).filter((e: string) => e.length > 0);
 if (entries.length < 1 || entries.length > 3) return { imageCount: true };

 const allJpg = entries.every((e: string) => e.split('|')[0].trim().toLowerCase().endsWith('.jpg'));
...
export class Products implements OnInit {
 ...
 error: (err) => {
 this.submitting.set(false);
 this.modalErrorMessage.set(err.error?.message ?? 'No se pudo guardar. Revisá los datos ingresados.');
 }
 });
 }
 ...
 get line() { return this.form.controls.line; }
