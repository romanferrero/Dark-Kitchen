...
export class Users {
 ...
 this.userService.create({ firstName: firstName!, lastName: lastName!, email: email!, phone: phone!, password: password!, role: role! }).subscribe({
 next: () => {
 ...
 error: (err) => {
 this.loading.set(false);
 this.errorMessage.set(err.error?.message ?? 'No se pudo crear el usuario. Revisá los datos ingresados.');
 }
 });
 }
